...
PLUGIN_NAME = "Generate Cuesheets"
PLUGIN_AUTHOR = "Lukáš Lalinský, Sambhav Kothari, Matt Goldspink"
PLUGIN_DESCRIPTION = "Generate per media cuesheets (.cue file) from an album."
...
class GenerateCuesheets(BaseAction):
    NAME = "Generate &Cuesheets..."
    ...
    def callback(self, objs):
        ...
        if filename:
            ...
            for i in range(int(album.metadata["totaldiscs"])):
                cuesheet = Cuesheet2(filename + ("%02d" % (i + 1)) + ".cue")

                try:
                    # always need one extra since the first one is
                    # for the album entry
                    track = CuesheetTrack2(cuesheet, len(cuesheet.tracks))
                    ...
                    t.set("REM", "DISCNUMBER", str(i + 1))
                    t.set("REM", "TOTALDISCS", album.metadata["totaldiscs"])
                    if "date" in album.metadata:
                        t.set("REM", "DATE", album.metadata["date"])
                    index = 0.0
                    for j, track in enumerate(album.tracks):
                        if int(track.metadata["discnumber"]) == i + 1:
                            ct = CuesheetTrack2(cuesheet, len(cuesheet.tracks))
                            cuesheet.tracks.append(ct)
                            mm = index / 60.0
                            ss = (mm - int(mm)) * 60.0
                            ff = (ss - int(ss)) * 75.0
                            index += track.metadata.length / 1000.0
                            t = cuesheet.tracks[len(cuesheet.tracks) - 1]
                            t.set("TRACK", track.metadata["tracknumber"], "AUDIO")
                            ...
                            t.set("REM", "DISCNUMBER", track.metadata["discnumber"])
                            t.set("REM", "TOTALDISCS", track.metadata["totaldiscs"])
                            ...
                            for file in track.linked_files:
                                ...
                                if extension in ["mp3", "mp2", "m2a"]:
                                    ...
                                    file_type = "WAVE"
                                cuesheet.tracks[len(cuesheet.tracks) - 1].set("FILE", audio_filename, file_type)

                        cuesheet.write()
                except Exception as e:
                    QtWidgets.QMessageBox.warning(None, getattr(e, 'message', str(e)), getattr(e, 'message', repr(e)))

action = GenerateCuesheets()
